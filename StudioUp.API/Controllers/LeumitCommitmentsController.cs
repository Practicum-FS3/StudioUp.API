...
namespace StudioUp.API.Controllers
{
    ...
    public class LeumitCommitmentsController: ControllerBase
    {
        ...
        public async Task<ActionResult<LeumitCommitmentsDTO>> add(LeumitCommitmentsDTO leumitCommiments)
        {
            try
            {
                 await leumitCommimentsRepository.AddAsync(leumitCommiments);
                return CreatedAtAction(nameof(add), leumitCommiments);
            }
            catch (Exception ex)
            {
                throw ex;
